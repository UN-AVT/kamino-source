...
# The specific countries to generate the facet
countries <- c("Finland", "Sweden", "Norway", "Denmark")
df_region <- df %>% filter(country %in% countries)
...
# View using flow & fixed scale
v1 <- ggplot(data = df_region, aes(x = year, y = value, alluvium = variable)) +
...
# View using flow & free y scale
v2 <- ggplot(data = df_region, aes(x = year, y = value, alluvium = variable)) +
  geom_flow(aes(fill = variable, size = value), colour = "#ffffff", alpha = 1.0, decreasing = FALSE) +
  # for set years
  scale_x_continuous(breaks = years) +
  # for aethetics to show top-down
  scale_y_reverse() +
  # use the palette defined above
  scale_fill_manual(values = energy_pal) +
  # scale line size so smallest values still show
...
# ggsave(filename = "alluvial-country-energy-consumption-facet-fixed.svg", plot = v1, width=12, height=6)
# ggsave(filename = "alluvial-country-energy-consumption-facet-free.svg", plot = v2, width=12, height=6)
