...
```{r, out.width = "60%", fig.align = 'center', fig.cap="Photo by Bruno on Unsplash"}

masthead <- "assets/bruno-unsplash.jpg"
...
rank_df <- rank_df %>% filter(YEAR %in% c(1990, 1995, 2000, 2005, 2010, 2015, 2018))
...
v1 <- ggplot(data = rank_df, aes(x = YEAR, y = RANK, group = COUNTRY)) +
  ...
  #             spline_shape = -.15) +
  geom_line(aes(color=ifelse(((COUNTRY %in% "China")),"Below", "Above")), size = 1.0) +
...
* Data Sources: Statistics Canada, [GO](https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=2410000601){target="_blank"}
