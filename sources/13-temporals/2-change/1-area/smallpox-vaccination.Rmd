...
df <- read.csv("archetypes/smallpox-vaccination/deaths-from-smallpox-in-london.csv", header = TRUE)
head(df, n=10)
...
v1 <- ggplot(data = df_wrangled, 
  ...
  geom_segment(aes(x = as.Date("1800-01-01"), y = 0, xend = as.Date("1800-01-01"), yend = 21), 
               linetype=2, color = "#ea4d48", size = 1) +
  geom_text(aes(x = as.Date("1800-01-01")+2*365, y = 21+0.5, label = "Cowpox vaccine\nintroduced"), 
