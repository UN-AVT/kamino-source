...
# Create cartogram
wrld_carto <- cartogram_dorling(wrld, "POP2005")

wrld_sf = st_as_sf(wrld)
wrld_sf_carto <- cartogram_dorling(wrld_sf, "POP2005")
...
v1 <- ggplot(data = wrld_sf) +
  # map feature styling
  geom_sf(fill="white", color="#666666", stroke=0.5) + 
  coord_sf() +
  theme_bw() +
  labs(x="Longitude",
       y="Latitude",
...
v3 <- ggplot() +
  # map feature styling
  geom_sf(data = wrld_sf, fill="white", color="#666666", stroke=0.5) + 
  geom_sf(data = wrld_sf_carto, aes(fill=POP2005, color=POP2005), stroke=0.5) +
...
ggsave(filename = "assets/dorling-cartogram.svg", plot = v3, width=12, height=6)
