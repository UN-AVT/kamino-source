...
pdf_file <- "./archetypes/un-speeches/diamonds-fuelling-conflict/A_55_PV.79-EN.pdf"
...
characters_count <- count_characters(pages_df$pages) %>% sum()
characters_count

words_count <- count_words(pages_df$pages) %>% sum()
words_count

sentences_count <- count_sentences(pages_df$pages) %>% sum()
sentences_count
...
text_corpus_lwr <- tm_map(text_corpus, content_transformer(tolower))
text_corpus_lwr

# Remove numbers
text_corpus_rn <- tm_map(text_corpus_lwr, removeNumbers)
text_corpus_rn

# Remove punctuations
text_corpus_rp <- tm_map(text_corpus_rn, removePunctuation)
text_corpus_rp
