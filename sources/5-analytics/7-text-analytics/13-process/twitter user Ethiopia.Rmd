...
```{r}
## You can use below method to lookup users by screen_name or user_id, and extract most recent tweets data from the users
#users <- c("UNEthiopia","UNHCREthiopia","OCHA_Ethiopia","UNDPEthiopia","WFP_Ethiopia","UNICEFEthiopia")

#tw<-get_timeline(users, n = 1000)

## We run the search and save the result as of Aug 24 in a dataset, only keep the first 10 columns, so you can ingest the dataset as below shows, or do any search that you are interested in. 
```

```{r}

tw<-read_csv("archetypes/ethiopia_user_tweets.csv")
head(tw)
```
