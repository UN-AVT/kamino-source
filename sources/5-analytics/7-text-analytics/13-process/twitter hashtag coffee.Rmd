...
```{r}
# Search for up to 10,000 tweets containing #coffee, excluding re-tweets:
...
coffee_tweets<-read_csv("archetypes/coffee_tweets.csv")
head(coffee_tweets)
```
...
coffee_tweets_text<-coffee_tweets%>%select(text)
coffee_tweets_text$stripped_text <- gsub("http.*","",  coffee_tweets_text$text)
coffee_tweets_text$stripped_text <- gsub("https.*","", coffee_tweets_text$stripped_text)

head(coffee_tweets_text)
```
## Tokenize
```{r}
# remove punctuation, convert to lowercase
coffee_tweets_text_clean <- coffee_tweets_text %>%
  dplyr::select(stripped_text) %>%
  unnest_tokens(word, stripped_text)
coffee_tweets_text_clean
```
...
cleaned_tweet_words <- coffee_tweets_text_clean %>%
...
bigram <- coffee_tweets_text %>% unnest_tokens(bigram, stripped_text, token = "ngrams", n = 2)
...
text <- paste(coffee_tweets_text$stripped_text[1:50], collapse = " ")
...
kwic_keyword <- kwic(coffee_tweets_text$stripped_text, pattern = keyword, valuetype = "regex")
#names(kwic_keyword)
