---
pagetitle: "Coronavirus evolution"
output:
  html_document:
    theme: lumen
...
```{r, out.width = "60%", fig.align = 'center', fig.cap="Photo by Gani Nurhakim on Unsplash"}

masthead <- "assets/gani-nurhakim-unsplash.jpg"
...
# Tree data
df_tree_path <- "archetypes/coronavirus-evolution/nextstrain_ncov_gisaid_global_timetree.nwk"
df_tree <- read.tree(df_tree_path)
df_tree
...
edges <- as.data.frame(df_tree$edge)
colnames(edges) <- c("from","to")
edges

# convert phylo structure to data frame
nodes <- as.treedata(df_tree) %>% as_tibble
# split label to columns
nodes <- nodes %>% separate(label, c("X1", "LOC", "BRANCH", "YEAR"), "/")
nodes <- nodes %>% select(node, branch.length, LOC, BRANCH, YEAR)
nodes
...
df_graph <- graph_from_data_frame( edges, vertices = nodes )
df_graph_directed <- graph_from_data_frame( edges, vertices = nodes, directed = TRUE )
df_graph_undirected <- graph_from_data_frame( edges, vertices = nodes, directed = FALSE )
...
v1 <- ggraph(df_graph, layout = backbone) + 
  # geom_edge_elbow(aes(label = LABEL), angle_calc = 'along', label_dodge = unit(2.5, 'mm')) + 
  # geom_edge_bend(aes(label = LABEL)) +
  geom_edge_diagonal() +
  geom_node_point(size=2) +
  # geom_node_text( aes(label=BRANCH), repel = FALSE, vjust = 0.0, nudge_y = 0.1, size = 3, family = "inconsolata") +
...
v3 <- ggraph(df_graph, layout = sugiyama) + 
  # geom_edge_elbow(aes(label = LABEL), angle_calc = 'along', label_dodge = unit(2.5, 'mm')) + 
  # geom_edge_bend(aes(label = LABEL)) +
  geom_edge_diagonal() +
  geom_node_point(size=2) +
...
v4 <- ggraph(df_graph, layout = dendrogram) + 
  geom_edge_elbow() +
  geom_node_point(size=2) +
  # geom_node_text( aes(label=BRANCH), repel = FALSE, vjust = 0.0, nudge_y = 0.1, size = 3, family = "inconsolata") +
  # coord_fixed(clip = 'off') + 
